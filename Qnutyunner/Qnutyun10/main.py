...
def feedback(random_number, user_input):
    ...
    for i in range(len(random_number)):
        ...
        if user_input[i] in random_number:
            number_in += 1
    return f"{number_in} թիվ կա գաղտնի կոդում, իսկ {number_in_places} թիվ իր տեղում է, գաղտնի թիվը։ {random_number}", False
...
def start(message):
    ...
    user_attempts.append(10)
    bot.reply_to(message, f"Բարի գալուստ, փորձեք գուշակել թիվը, ձեզ մնացելե {user_attempts[0]} փորձ։ Ձեր միավորները տեսնելու համար սեղմեք /points ի վրա")
